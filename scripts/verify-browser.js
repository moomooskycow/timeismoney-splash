...
// Exercises js/sentry.js in a vm sandbox with a fake DOM: proves the
...
//
// Usage: node scripts/verify-browser.js
...
const SOURCE = fs.readFileSync(path.join(ROOT, 'js', 'sentry.js'), 'utf8');
...
async function verifyDeferredUntilDomReady() {
  const { sandbox, fetches, listeners } = makeSandbox({
    config: { enabled: false, service: 'timeismoney-splash', environment: 'production', release: null },
    readyState: 'loading',
  });
  run(SOURCE, sandbox);
  await flush();
...
async function main() {
  ...
  console.log('timeismoney browser bootstrap verification passed');
}
